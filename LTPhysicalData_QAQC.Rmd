...
if(!require(tidyverse)) {install.packages("tidyr")
  library(tidyverse)}
if(!require(gridExtra)) {install.packages("tidyr")
  library(gridExtra)}
if(!require(kableExtra)) {install.packages("tidyr")
  library(kableExtra)}
if(!require(knitr)) {install.packages("tidyr")
  library(knitr)}
...
VisHist <-  function(data,x, bin) {
    x <- enquo(x)
  data %>%
    ggplot() +
    geom_histogram(mapping = aes(!! x,col = StationCode),binwidth = bin, fill = "lightseagreen", colour = "lightgray") +
