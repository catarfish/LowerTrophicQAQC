...
# Allow  2 days after last "inundation=TRUE" to also count as inundated. Merge with sample data. 
inundation4 <- inundation %>%
  mutate(Inundation2 = ifelse(lead(Inundation, 2) == "TRUE", "TRUE", Inundation)) %>%
  select(c(Date, Month:Inundation2))

samp_catch_phys <- left_join(samp_catch_phys0, inundation4)
...
17. Make a table of summarized flowmeter values
...
Flow.sum.STTD <- samp2 %>%
  ...
  filter(!is.na(Flowdiff), !is.na(Inundation2), StationCode=="STTD") %>%
  mutate(Flow_s = Flowdiff/SetTime) %>%
  group_by(StationCode, FlowMeterSpeed, WYClass, Inundation2) %>%
  summarize(n= n(),
...
Flow.sum.SHR <- samp2 %>%
  ...
  filter(!is.na(Flowdiff), StationCode == "SHR") %>%
  summarize(n= n(),
            min.Flowdiff = min(Flowdiff),
            max.Flowdiff = max(Flowdiff),
            median.Flowdiff = median(Flowdiff),
...
# If Flag 3, calculate Flowdiff_adj = median.Flowdiff_s * SetTime
...
25. Calculate Vol and CPUE, Vol_adj, CPUE_adj
...
# SHR vs STTD boxplot by month
CPUEBoxMonth <- ggplot(FlowQAQC_CPUE) + geom_boxplot(aes(x = Month, y = CPUE_adj, color = Month)) + facet_wrap(~Station) + scale_color_viridis(discrete = TRUE) + theme_bw() 
...
checkCPUE2 <- filter(FlowQAQC_CPUE, is.na(CPUE_adj))
```

28. Look at summary table of CPUE values
```{r}
### Table of values
...
outlier2b <- ggplot(CPUEQAQC, aes(x = Month, y = CPUE_adj, color = Flag_CPUE)) + geom_point(size = 2.5, alpha = 0.5) + scale_color_viridis(discrete = "TRUE") + facet_wrap(~Station) +   labs(title = "CPUE calculated with adjusted Flowmeter values + CPUE flags") + 
  theme_bw() + theme(axis.text = element_text(size = 14),
                     axis.title = element_text(size =14),
                     legend.text = element_text(size = 14),
                     legend.title = element_text(size = 14), 
...
inv_select <- CPUEQAQC %>%
  ...
  select(c(Datetime, StationCode,
           ...
           SetTime:FlowMeterEnd, Flowdiff, Flowdiff_adj, Volume, Volume_adj,
           LabComments, 
           TaxonName:LifeStage, CPUE, CPUE_adj, 
           Flag_PQC, Comment_PQC, Flag_SAMP, Comment_SAMP, Flag_LAB, Comment_LAB, Flag_FM, Comment_FM, Flag_CPUE, Comment_CPUE)) 
...
inv_final <- inv_select %>%
  mutate_each(list(empty_as_na), Flag_SAMP:Comment_CPUE) %>%
...
write.csv(inv_final, paste0("R_write/Drift/Drift_data_", today, ".csv"), row.names = FALSE)
```
