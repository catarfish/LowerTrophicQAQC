...
zoop_tax <- read_csv("Data/TblZoopsLookupV2.csv")
yb_tax <- read.csv("Data/YB_Taxonomy_2.csv")
kieco <- read.csv("Data/ZOOP_kieco_taxonomy.csv")
...
# Change spellings
# yb_tax$Organism[yb_tax$Organism=="Ergasilus sp."] <- "Ergasilus spp."
# yb_tax$Organism[yb_tax$Organism=="Skistodiaptomus sp."] <- "Skistodiaptomus spp."
# yb_tax$Organism[yb_tax$Organism=="Tropocyclops sp."] <- "Tropocyclops spp."
# yb_tax$Organism[yb_tax$Organism=="Acanthocyclops sp."] <- "Acanthocyclops spp."
# yb_tax$Organism[yb_tax$Organism=="Pseudochydorus sp."] <- "Pseudochydorus spp."
# yb_tax$Organism[yb_tax$Organism=="Oithona Spp."] <- "Oithona spp."
...
zoop_all_1 <- left_join(zoop_all_0, yb_tax, by = "OrganismID")

# Rename some columns, remove unnecessary columns
zoop_all_2 <- zoop_all_1 %>%
  dplyr::select(-c( X, MicrocystisVisualRank, StartTime:`50_StopTime`, EnteredBy:SlideCount, 
             ZoopsCatchID, ZoopsCode, Organism.x, Classification.x, Comments,
  ...
  rename(`150_VolMicro`= VolMicro,
         ...
         Organism = Organism.y,
         Classification = Classification.y)

# Remove taxonomy and replace with taxonomy from another file (below)
zoop_all_3 <- zoop_all_2 %>%
  dplyr::select(-c(46:54))

# Taxonomy from Kieco
kieco_ed <- kieco %>%
  filter(str_detect(Databases.present, "ZOOP"))%>%
  dplyr::select(c(1:16,21:22))

kieco_m <- left_join(zoop_all_3, kieco_ed, by= c("TaxonName")) %>%
  filter(!(OrganismID==64 & OrganismCode_ZOOP==68)) %>%
  filter(!(OrganismID==68 & OrganismCode_ZOOP==64))


# Rearrange columns
zoop_all <- kieco_m[, c("PhysicalDataID", "ZoopsDataID", 
                         ...
                         "Subsample.", "OrganismID", "OrganismCode_ZOOP", "Classification", "Organism", "Phylum",
