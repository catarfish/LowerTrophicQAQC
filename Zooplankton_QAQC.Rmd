...
zoop_all_1 <- zoop_all_0 %>%
  dplyr::select(-c(StartTime:`50_StopTime`, EnteredBy:SlideCount, 
                   ZoopsCatchID, ZoopsCode,`50_V_reg`, `50_V_low`, `150_V_reg`, `150_V_low`, 
                   `50_S_micro`, `50_S_meso`, `150_S_micro`, `150_S_meso`)) %>%
...
# Flag Flowmeter data
zoop_forreview <- zoop_forreview_0 %>%
  mutate(QC_Flag1 = ifelse(((`150_Flowdiff` < 400 & `150_Flowdiff` > 199) | 
                             (`150_Flowdiff` > 9999 & `150_Flowdiff`<20000)), 3, 
                    ifelse((`150_Flowdiff` <200 | `150_Flowdiff` > 19999), 5, ""))) %>%
  mutate(QC_Comment1 = ifelse(`150_Flowdiff`< 400, "FMLOW",
                              ifelse(`150_Flowdiff` > 9999 , "FMHIGH", "")))

## Fix 50 data
  
  (`50_Flowdiff` > 99 & `50_Flowdiff` < 200) |
                             (`50_Flowdiff` > 4999 & `50_Flowdiff`<10000))
  50_Flowdiff` <100 | `50_Flowdiff` > 9999),
`50_Flowdiff`< 200
| `50_Flowdiff` > 4999

# Add QC SAMP info 
qcsamp_sub <- qcsamp %>% select(c("ZoopsDataID", "QC_Flag2", "QC_Comment3", "ZoopNetType"))
zoop_forreview <- left_join(zoop_forreview_0, qcsamp_sub, by = c("ZoopsDataID", "ZoopNetType"))

zoop_forreview$Date <- as.Date(zoop_forreview$Date, format = "%m/%d/%y")
